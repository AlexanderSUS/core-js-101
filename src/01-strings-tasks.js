...
function extractNameFromTemplate(value) {
  return value.slice('Hello, '.length).slice(0, -1);
}
...
function encodeToRot13(str) {
  return str.split('').map((symbol) => {
    if (symbol.match(/[A-Za-z]/)) {
      if (symbol.toUpperCase().charCodeAt(0) + 13 > 'Z'.charCodeAt(0)) {
        const position = symbol.toUpperCase().charCodeAt(0) - 'A'.charCodeAt(0);
        const rest = (symbol.toUpperCase().charCodeAt(0) + 12) % 'Z'.charCodeAt(0);
        return String.fromCharCode(symbol.charCodeAt(0) - position + rest);
      }
